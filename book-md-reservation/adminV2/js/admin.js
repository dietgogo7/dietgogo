document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DOM ELEMENTS ---
    // --- CONFIGURATION ---
    // 스케줄링 관련 설정 (유연한 시간대 및 요일)
    const SCHEDULE_CONFIG = {
        selectableDays: [1, 2, 3, 4], // 0:Sun ~ 6:Sat (Current: Mon-Thu)
        timeSlots: {
            start: "14:00", // Start time (HH:MM)
            end: "15:30",   // End time (HH:MM) - Inclusive of last slot start if interval permits
            intervalMinutes: 15
        }
    };
    ...
    function renderAdminCalendar() {
        ...
        for (let d = 1; d <= lastDate; d++) {
            ...
            // Check Config Days AND Holidays
            const isConfigDay = SCHEDULE_CONFIG.selectableDays.includes(dayOfWeek);
            const holidayName = window.HolidayManager ? window.HolidayManager.getHolidayName(dateStr) : null;
            const isHoliday = !!holidayName;

            const isAllowedDay = isConfigDay && !isHoliday;
            ...
            if (dayOfWeek === 0 || isHoliday) classes.push('sunday'); // Red for Sunday or Holiday
            if (dayOfWeek === 6 && !isHoliday) classes.push('saturday'); // Blue for Saturday (unless Holiday)
            ...
            const cursorStyle = isAllowedDay ? '' : 'style="cursor: not-allowed;"';
            // Show holiday name as tooltip title if exists
    ...
    function renderScheduleGrid() {
        ...
        // Check if date is valid based on Config & Holidays & Past
        const d = new Date(date + 'T00:00:00');
        const day = d.getDay();
        const isConfigDay = SCHEDULE_CONFIG.selectableDays.includes(day);
        ...
        const isHoliday = window.HolidayManager ? window.HolidayManager.isHoliday(d) : false;
        ...
        const startMin = parseTime(SCHEDULE_CONFIG.timeSlots.start);
        const endMin = parseTime(SCHEDULE_CONFIG.timeSlots.end);

        // Loop from startMin to endMin inclusive of start time step
        const slotDescriptors = [];
        for (let t = startMin; t <= endMin; t += SCHEDULE_CONFIG.timeSlots.intervalMinutes) {
    ...
    function renderBlacklist() {
        ...
        sortedList.forEach(item => {
            ...
            row.innerHTML = `
                <td>${item.Phone_No}</td>
                <td>${item.Reason}</td>
                <td style="text-align:center;">${formatWithYear(item.Regist_Datetime)}</td>
                <td>
