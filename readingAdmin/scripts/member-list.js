...
$(document).ready(function () {
  loadSidebar('member-list');

  $.getJSON('datas/tReadingRunMember.json', function (data) {
    _allMembers = data;
    doSearch();
  });
...
function renderPage(page) {
  ...
  for (var i = start; i < end; i++) {
    ...
    rows += '<tr>' +
      '<td>' + (i + 1) + '</td>' +
      '<td><a href="member-detail.html?memberNo=' + m.Member_No + '" class="link-primary">' + m.Member_Id + '</a></td>' +
